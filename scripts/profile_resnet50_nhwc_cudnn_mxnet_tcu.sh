...
RESULTS_DIR=profile_results/mxnet_tcu_nhwc/cudnn_log/${GPU_NAME}
...
for BATCH_SIZE in "${batch_sizes[@]}"; do
    ...
    CUDNN_LOGINFO_DBG=1 CUDNN_LOGDEST_DBG=${CUDNN_LOGDEST_DBG_PATH} python tcu/run_mxnet_nhwc.py ${BATCH_SIZE_OPT} --num_warmup=$NUM_WARMUP --num_iterations=$NUM_ITERATIONS --model_idx=$MODEL_IDX
done
